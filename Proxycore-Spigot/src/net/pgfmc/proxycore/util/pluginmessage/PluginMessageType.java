...
public enum PluginMessageType {
	...
	// pgf:main
	CONNECT(PluginMessage.CHANNEL_PGF, "Connect", PluginMessageMatchType.NONE),
	 ...
	 */
	public final CompletableFuture<List<String>> send(final Player sender, final List<String> args)
	{
		Logger.debug("------------------------------");
		...
		// send plugin message
		sender.sendPluginMessage(Main.plugin, channel, out.toByteArray());
	 ...
	 * @return A CompletableFuture that holds an ordered List<String> of the plugin message response (if any)
	 */
	private static final CompletableFuture<List<String>> createFuture(final PluginMessageType originalType, final Player originalPlayer, final List<String> originalArgs)
	{
		// the future
		...
		final Thread thread = new Thread(new Runnable() {
			...
			class FuturePluginMessage extends PluginMessage {
				...
				final Player originalSender;
				final List<String> originalArgs;

				public FuturePluginMessage(final Player originalSender, final List<String> originalArgs) {
				...
				public void onPluginMessageReceived(final Player sender, final List<String> args) {
					...
					case ARGUMENT: // Check original argument with this argument
						if (args.size() < 2 || originalArgs.size() < 1)
						{
							...
							Logger.error("Original Sender: " + originalSender.getName());
			...
			@Override
			public void run() {
				new FuturePluginMessage(originalPlayer, originalArgs);
	...
	public final CompletableFuture<List<String>> send(final Player sender, final String arg0, final String arg1, final String arg2, final String arg3)
	{
		return send(sender, Arrays.asList(arg0, arg1, arg2, arg3));
	}
	
	public final CompletableFuture<List<String>> send(final Player sender, final String arg0, final String arg1, final String arg2)
	{
		return send(sender, Arrays.asList(arg0, arg1, arg2));
	}
	
	public final CompletableFuture<List<String>> send(final Player sender, final String arg0, final String arg1)
	{
		return send(sender, Arrays.asList(arg0, arg1));
	}
	
	public final CompletableFuture<List<String>> send(final Player sender, final String arg0)
	{
		return send(sender, Arrays.asList(arg0));
	}
	
	public final CompletableFuture<List<String>> send(final Player sender)
	{
