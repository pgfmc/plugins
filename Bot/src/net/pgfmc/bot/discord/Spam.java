...
public class Spam {
	...
	public static void check(Player p)
	{
		...
		if (!Punish.isMute(pd) && !pd.hasPermission("pgf.spam.exempt") && new Date().getTime() - (long) Optional.ofNullable(pd.getData("spam.lastchat")).orElse(1000L) <= 550)
		{
			...
			if (warnings >= 3 && !pd.hasPermission("pgf.spam.exempt"))
			{
				...
				Discord.sendAlert(Discord.simplePlayerEmbed(pd.getPlayer(), "has been muted for spamming.", Colors.RED).build()).queue();
	...
	private static void addTimer(PlayerData pd)
	{
		Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, new Runnable() {
			public void run() {
				Discord.sendAlert(Discord.simplePlayerEmbed(pd.getPlayer(), "has been unmuted.", Colors.GREEN).build()).queue();
